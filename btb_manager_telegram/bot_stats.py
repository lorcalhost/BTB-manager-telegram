...
try:
    ...
    for i in range(1, lenTradeHistory):
        db.execute('SELECT alt_coin_id FROM trade_history where id=\'{}\' and state=\'COMPLETE\' order by id asc limit 1'.format(i))
        coinID = db.fetchall()
        if len(coinID) > 0:
            coinID = coinID[0][0]
        else:
        ...
        for coin in coinList:
            if coinID == coin[0]:
                ...
                db.execute('select alt_trade_amount from trade_history where alt_coin_id=\'{}\' and state=\'COMPLETE\' order by id asc limit 1'.format(initialCoinID))
                initialCoinValue = db.fetchall()[0][0]

                db.execute('select crypto_trade_amount from trade_history where alt_coin_id=\'{}\' and state=\'COMPLETE\' order by id asc limit 1'.format(initialCoinID))
    ...
    db.execute('select alt_trade_amount from trade_history where selling=0 and state=\'COMPLETE\' order by id desc limit 1')
    ...
    if lastCoinID != initialCoinID and initialCoinID != '':
        db.execute("select id from pairs where from_coin_id=\'{}\' and to_coin_id=\'{}\'".format(lastCoinID, initialCoinID))
        pairID = db.fetchall()[0][0]
        db.execute("select other_coin_price from scout_history where pair_id=\'{}\' order by id desc limit 1".format(pairID))
    ...
    for coin in coinList:
        jumps = db.execute(f"select count(*) from trade_history where alt_coin_id=\'{coin[0]}\' and selling=0 and state=\'COMPLETE\'").fetchall()[0][0]
        if jumps > 0:
            multiTrades += jumps
            first_date = db.execute(f'select datetime from trade_history where alt_coin_id=\'{coin[0]}\' and selling=0 and state=\'COMPLETE\' order by id asc limit 1').fetchall()[0][0]
            first_value = db.execute(f'select alt_trade_amount from trade_history where alt_coin_id=\'{coin[0]}\' and selling=0 and state=\'COMPLETE\' order by id asc limit 1').fetchall()[0][0]
            last_value = db.execute(f'select alt_trade_amount from trade_history where alt_coin_id=\'{coin[0]}\' and selling=0 and state=\'COMPLETE\' order by id desc limit 1').fetchall()[0][0]
            grow = (last_value - first_value)/first_value*100
            x.add_row([coin[0], '{:.2f}'.format(first_value), '{:.2f}'.format(last_value), '{:.1f}'.format(grow),'{}'.format(jumps)])
