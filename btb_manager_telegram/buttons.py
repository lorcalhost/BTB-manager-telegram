...
def current_value():
    ...
    message = [i18n_sane("database_not_found", path=db_file_path)]
    if os.path.exists(db_file_path):
        try:
            ...
            try:
                ...
                if state == "ORDERED":
                    return [
                        f"{i18n_sane('order_placed', order_size=format_float(order_size), bridge=bridge, current_coin=current_coin)}\n\n"
                        f"{i18n_sane('wait_for_order')}"
                    ]
                ...
                con.close()
                return [i18n_sane("fetch_coin_from_db_error")]
            ...
            try:
                ...
                if query is None:
                    return [
                        i18n_sane("no_information", current_coin=current_coin),
                        i18n_sane("no_current_value_during_trade"),
                    ]
                ...
                con.close()
                return [
                    i18n_sane("fetch_coin_from_db_error"),
                    i18n_sane("no_current_value_during_trade"),
                ]
            ...
            try:
                m_list = [
                    f"\n{i18n_sane('last_update', update=last_update.strftime('%H:%M:%S %d/%m/%Y'))}\n\n",
                    f"{i18n_sane('current_coin', coin=current_coin)}\n",
                    f"\t{i18n_sane('balance', balance=format_float(balance), coin=current_coin)}\n",
                    f"\t{i18n_sane('exchange_rate_purchased', rate=format_float(buy_price / alt_amount), bridge=bridge, coin=current_coin)}\n",
                    f"\t{i18n_sane('exchange_rate_now', rate=format_float(usd_price), coin=current_coin)}\n",
                    f"\t{i18n_sane('value_change', change=round((balance * usd_price - buy_price) / buy_price * 100, 2))}\n",
                    f"\t{i18n_sane('value_usd', value=round(balance * usd_price, 2))}\n",
                    f"\t{i18n_sane('value_btc', value=round(balance * btc_price))}\n\n",
                    f"{i18n_sane('bought_for', value=round(buy_price, 2), coin=bridge)}\n"
                    f"{i18n_sane('one_day_change_btc', value=return_rate_1_day)}\n",
                    f"{i18n_sane('seven_day_change_btc', value=return_rate_7_day)}\n",
                ]
                ...
                con.close()
                return [i18n_sane("error_generating_value")]
        except Exception as e:
            logger.error(
                f"❌ Unable to perform actions on the database: {e}", exc_info=True
            )
            message = [i18n_sane("db_action_error")]
    return message
    ...
    if os.path.exists(db_file_path):
        try:
            ...
            try:
                ...
                # Generate message
                m_list = [f"{i18n_sane('coin_progress')}\n\n"]
                for coin in query:
                    ...
                    m_list.append(
                        f"*{coin[0]}*\n"
                        f"\t{i18n_sane('amount', amount=format_float(coin[1]), coin=coin[0])}\n"
                        f"\t{i18n_sane('price', amount=round(coin[2], 2))}\n"
                        f"\t{change}\n"
                        f"\t{i18n_sane('trade_datetime', date=last_trade_date)}\n\n"
                    )
                ...
                con.close()
                return [i18n_sane("progress_fetch_error")]
        except Exception as e:
            logger.error(
                f"❌ Unable to perform actions on the database: {e}", exc_info=True
            )
    ...
    if os.path.exists(db_file_path):
        try:
            ...
            try:
                ...
                m_list = [
                    f"\n{i18n_sane('last_update', update=last_update.strftime('%H:%M:%S %d/%m/%Y'))}\n\n"
                    f"{i18n_sane('compared_ratios', coin=current_coin)}\n"
                ]
                for coin in query:
                    m_list.append(
                        f"*{coin[1]}*:\n"
                        f"\t{i18n_sane('bridge_value', value=coin[2], bridge=bridge)}\n"
                        f"\t{i18n_sane('ratio', ratio=format_float(coin[3]))}\n\n"
                    )
                ...
                con.close()
                return [
                    i18n_sane("ratio_gen_error"),
                    i18n_sane("logging_enabled_error"),
                ]
        except Exception as e:
            logger.error(
                f"❌ Unable to perform actions on the database: {e}", exc_info=True
...
def next_coin():
    ...
    db_file_path = os.path.join(settings.ROOT_PATH, "data/crypto_trading.db")
    user_cfg_file_path = os.path.join(settings.ROOT_PATH, "user.cfg")
    message = [f"{i18n_sane('database_not_found', path=db_file_path)}"]
    if os.path.exists(db_file_path):
        try:
            # Get bridge currency symbol
            with open(user_cfg_file_path) as cfg:
            ...
            try:
                ...
                for coin in query:
                    ...
                    m_list.append(
                        ...
                        f"\t{i18n_sane('current_price', price=format_float(round(coin[1], 8)), coin=bridge)}\n"
                        f"\t{i18n_sane('target_price', price=format_float(round(coin[2], 8)), coin=bridge)}\n\n"
                    )
...
def check_status():
    ...
    message = i18n_sane("bot_not_running")
    if get_binance_trade_bot_process():
        message = i18n_sane("bot_running")
    return message
    ...
    if os.path.exists(db_file_path):
        try:
            ...
            try:
                ...
                m_list = [
                    f"{i18n_sane('last_x_trades', trades=10 if len(query) > 10 else len(query))}\n\n"
                ]
                for trade in query:
                    if trade[4] is None:
                        continue
                    ...
                    m_list.append(
                        ...
                        f"{trade_details}\n"
                        f"{i18n_sane('trade_status', status=trade[3])}\n\n"
                    )
                ...
                con.close()
                return [i18n_sane("trade_history_error")]
        except Exception as e:
            logger.error(
                f"❌ Unable to perform actions on the database: {e}", exc_info=True
            )
...
def start_bot():
    ...
    message = i18n_sane("bot_already_running")
    if not get_binance_trade_bot_process():
        if os.path.isfile(settings.PYTHON_PATH):
            if os.path.exists(os.path.join(settings.ROOT_PATH, "binance_trade_bot/")):
                ...
                if get_binance_trade_bot_process():
                    message = i18n_sane("bot_started")
                else:
                    message = i18n_sane("bot_start_error")
            else:
                message = (
                    f"{i18n_sane('installation_path_error', path=settings.ROOT_PATH)}\n"
                    f"{i18n_sane('directory_hint')}"
                )
        else:
            message = f"{i18n_sane('python_lib_error', path=settings.PYTHON_PATH)}\n"
    return message
...
def stop_bot():
    ...
    if get_binance_trade_bot_process():
        find_and_kill_binance_trade_bot_process()
        if not get_binance_trade_bot_process():
            message = i18n_sane("stopped_bot")
        else:
            message = f"{i18n_sane('stop_error')}\n\n" f"{i18n_sane('windows_hint')}"
    return message
...
def read_log():
    ...
    message = f"{i18n_sane('log_file_error', path=log_file_path)}"
    if os.path.exists(log_file_path):
        with open(log_file_path) as f:
            file_content = f.read()[-4000:]
            message = (
                f"{i18n_sane('last_4000_characters')}\n\n"
                f"```\n"
                f"{file_content}\n"
                f"```"
            )
...
def delete_db():
    ...
    message = i18n_sane("stop_bot_before_delete")
    delete = False
    db_file_path = os.path.join(settings.ROOT_PATH, "data/crypto_trading.db")
    if not get_binance_trade_bot_process():
        if os.path.exists(db_file_path):
            message = i18n_sane("sure_delete")
            delete = True
        else:
            message = f"{i18n_sane('database_not_found', path=db_file_path)}"
    print(message)
...
def edit_user_cfg():
    ...
    message = i18n_sane("stop_bot_before_edit_config")
    edit = False
    user_cfg_file_path = os.path.join(settings.ROOT_PATH, "user.cfg")
    if not get_binance_trade_bot_process():
        if os.path.exists(user_cfg_file_path):
            with open(user_cfg_file_path) as f:
                message = (
                    f"{i18n_sane('config_file_is')}\n\n"
                    f"```\n"
                    f"{f.read()}\n"
                    f"```\n\n"
                    f"{i18n_sane('reply_config')}\n\n"
                    f"{i18n_sane('stop_to_stop')}"
                )
                edit = True
        else:
            message = f"{i18n_sane('config_file_error', path=user_cfg_file_path)}"
    return [message, edit]
...
def edit_coin():
    ...
    message = i18n_sane("stop_bot_before_edit_coin_list")
    edit = False
    ...
    if not get_binance_trade_bot_process():
        if os.path.exists(coin_file_path):
            with open(coin_file_path) as f:
                message = (
                    f"{i18n_sane('coin_list_is')}\n\n"
                    f"```\n{f.read()}\n```\n\n"
                    f"{i18n_sane('reply_coin_list')}\n\n"
                    f"{i18n_sane('stop_to_stop')}"
                )
                edit = True
        else:
            message = f"{i18n_sane('coin_list_error', path=coin_file_path)}"
    return [message, edit]
...
def export_db():
    ...
    message = i18n_sane("stop_bot_before_export")
    db_file_path = os.path.join(settings.ROOT_PATH, "data/crypto_trading.db")
    file = None
    if not get_binance_trade_bot_process():
        if os.path.exists(db_file_path):
            with open(db_file_path, "rb") as db:
                file = db.read()
            message = i18n_sane("file_msg")
        else:
            message = i18n_sane("database_export_error")
    return [message, file]
...
def update_tg_bot():
    ...
    message = i18n_sane("tg_bot_up_to_date")
    upd = False
...
def update_btb():
    ...
    message = i18n_sane("btb_up_to_date")
    upd = False
...
def panic_btn():
    ...
    if not os.path.exists(db_file_path):
        return [i18n_sane("panic_db_error"), -1]

    user_cfg_file_path = os.path.join(settings.ROOT_PATH, "user.cfg")
    if not os.path.exists(user_cfg_file_path):
        return [i18n_sane("panic_config_error"), -1]
        ...
        try:
            ...
            if not selling:
                ...
                if state == "COMPLETE":
                    con.close()
                    return [
                        f"{i18n_sane('holding', amount1=format_float(round(alt_trade_amount, 6)), coin1=alt_coin_id, amount2=format_float(round(crypto_trade_amount, 2)), coin2=crypto_coin_id)}\n\n "
                        f"{i18n_sane('rate_when_bought')}\n"
                        f"`{format_float(round(price_old, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('current_rate')}\n"
                        f"`{format_float(round(price_now, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('current_value')}\n"
                        f"`{format_float(round(price_now * alt_trade_amount, 4))}` *{crypto_coin_id}*\n\n"
                        f"{i18n_sane('change')}\n"
                        f"`{format_float(round((price_now - price_old) / price_old * 100, 2))}` *%*\n\n"
                        f"{i18n_sane('stop_and_sell')}",
                        BOUGHT,
                    ]
                else:
                    con.close()
                    return [
                        f"{i18n_sane('open_buy_order', amount1=format_float(alt_trade_amount), coin1=alt_coin_id, amount2=format_float(crypto_trade_amount), coin2=crypto_coin_id)}\n\n"
                        f"{i18n_sane('limit_buy_price')}\n"
                        f"`{format_float(round(price_old, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('current_rate')}\n"
                        f"`{format_float(round(price_now, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('change')}\n"
                        f"`{format_float(round((price_now - price_old) / price_old * 100, 2))}` *%*\n\n"
                        f"{i18n_sane('stop_and_cancel')}",
                        BUYING,
                    ]
            else:
                if state == "COMPLETE":
                    con.close()
                    return [
                        f"{i18n_sane('order_already_complete', coin=crypto_coin_id)}\n\n"
                        f"{i18n_sane('ask_stop_bot')}",
                        SOLD,
                    ]
                else:
                    price_old = crypto_trade_amount / alt_trade_amount
                    ...
                    con.close()
                    return [
                        f"{i18n_sane('open_sell_order', amount1=format_float(alt_trade_amount), coin1=alt_coin_id, amount2=format_float(crypto_trade_amount), coin2=crypto_coin_id)}\n\n"
                        f"{i18n_sane('limit_sell_price')}\n"
                        f"`{format_float(round(price_old, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('current_rate')}\n"
                        f"`{format_float(round(price_now, 4))}` *{crypto_coin_id}*/*{alt_coin_id}*\n\n"
                        f"{i18n_sane('change')}\n"
