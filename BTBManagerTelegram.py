...
from src.SQLiteQuery import SQLiteQuery
    ...
    def __update_btb(self, update: Update, _: CallbackContext) -> int:
        ...
        if update.message.text != "Cancel update":
            ...
            try:
                ...
                subprocess.call(
                    f"cd {self.root_path} && git pull && $(which python3) -m pip install -r requirements.txt && $(which python3) -m binance_trade_bot &; gunicorn binance_trade_bot.api_server:app -k eventlet -w 1 --threads 1 -b 0.0.0.0:5123 &",
                    shell=True,
                )
            except:
    ...
    def __btn_start_bot(self):
        ...
        if not self.__find_process():
            if os.path.exists(f"{self.root_path}binance_trade_bot/"):
                ...
                subprocess.call(
                    f"cd {self.root_path} && gunicorn binance_trade_bot.api_server:app -k eventlet -w 1 --threads 1 -b 0.0.0.0:5123 &",
                    shell=True,
                )
                if not self.__find_process():
